...
test('view-only lineup refreshes when the page regains focus', async ({ page }) => {
  ...
  await expect(page.locator('.parent-game-card .fieldstar-logo-lockup')).toHaveCount(1)
  await expect(page.locator('.parent-game-card')).toContainText('EVERY KID PLAYS THE FIELD')
  await expect(page.locator('.parent-team-logo')).toHaveCSS('border-radius', '999px')
  expect(await page.locator('.parent-team-logo').evaluate((element) => element.getBoundingClientRect().width)).toBeGreaterThanOrEqual(64)
  expect(await page.locator('.parent-field-diamond').evaluate((element) => getComputedStyle(element, '::before').clipPath)).toContain('polygon')
